...
class compartmentModel(object):
    ...
    def solveODE(
        ...
        solverOpts, volumes, fluxes, flux_IDs, inletFlowrates, inletOrigins,
        ...
        for ID in range(self.nZones):
            chunks[ID % numOfCores].append(ID)
        ...
        while t < finalTime - timeTol:
            ...
            for k in range(self.nEnv):
                # constantVector = np.zeros(nZones)
            ...
            for worker, result in enumerate(results):
                ...
                for i, ID in enumerate(chunks[worker]):
                    sol_y = y_worker[i]
                    p_new[ID, :] = sol_y[:3, -1]
                    conc_new[ID, :] = sol_y[3:9, -1]
                    mom_new[ID, :] = sol_y[9:, -1]
    ...
    def integrateSource(
        self_ref, chunk_IDs, p, conc, mom, deltaT, solverOpts):
        ...
        for ID in chunk_IDs:
            ...
            sol = solve_ivp(self_ref.source, (0, deltaT), y0,
                            t_eval=[deltaT], args=(ID, self_ref, ),
                            vectorized=True, **solverOpts)

            y_chunks.append(sol.y)
            status_chunks.append(sol.status)

        return y_chunks, status_chunks

    @staticmethod
    def source(t, y_vec, ID, self):
        ...
        dtdy = np.zeros(y_vec.shape)

        for nn in range(y_vec.shape[1]):
            y = y_vec[:, nn]
            env_p = y[:nEnv]
            ...
            if reactEnv_p > 0:
                ...
                if np.all(concs[0:3] > effConc):
                    if concs[3] > effConc:
                        ...
                        superSat, _, equilConcs = self.equilibria.solve(concs, guess)
                    else:
                        conc_OH = concs[4]
                        k_sp_NMC = 1
                        powConcs_NMC = 1
                        for i, (k_sp_i, cationConcRatio) in enumerate(
                                zip(self.equilibria.k_sp, cationConcRatios)):
            ...
            dtdy[:, nn] = np.concatenate((dpdts, dcdts, dmdts))
